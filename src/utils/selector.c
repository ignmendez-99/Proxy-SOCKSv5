    ...
    if(s != NULL) {
        if(s->fds != NULL) {
            ...
            for(struct blocking_job *j = s->resolution_jobs; j != NULL; ) {
                struct blocking_job *next = j->next;
                free(j);
                j = next;
            }
            free(s->fds);
...
selector_status
selector_set_interest_additive(fd_selector s, int fd, fd_interest i) {
    selector_status ret = SELECTOR_SUCCESS;
    ...
    if(item->interest == OP_NOOP) {
        // no tenia ningun interes. Vamos a darle su primer interes
        item->interest = i;
        items_update_fdset_for_fd(s, item);
        goto finally;
    }

    if((item->interest == OP_READ && i == OP_WRITE) ||
        (item->interest == OP_WRITE && i == OP_READ)) {
        // El FD tenia uno de los 2 intereses. Vamos a añadirle el 2do interes sin eliminar el que ya tenia
        item->interest = OP_WRITE | OP_READ;
        items_update_fdset_for_fd(s, item);
    }
finally:
    return ret;
...
selector_status
selector_set_interest_reduction(fd_selector s, int fd, fd_interest i) {
    selector_status ret = SELECTOR_SUCCESS;
    ...
    if(item->interest == (OP_READ | OP_WRITE)) {
        // El FD tenia los 2 intereses. Vamos a sacarle solamente el interes "i"
        if(i == OP_READ)
            item->interest = OP_WRITE;
        else if(i == OP_WRITE)
            item->interest = OP_READ;
        items_update_fdset_for_fd(s, item);
    } else if((item->interest == OP_READ && i == OP_READ) || (item->interest == OP_WRITE && i == OP_WRITE)) {
        // El FD tenia solamente el interes que justo queremos remover. Vamos a removerselo
        item->interest = OP_NOOP;
        items_update_fdset_for_fd(s, item);
    }
                ...
                for(int i = 0 ; i < s->max_fd; i++) {
                    if(FD_ISSET(i, &s->master_r)|| FD_ISSET(i, &s->master_w)) {
                        if(-1 == fcntl(i, F_GETFD, 0)) {
                            proxy_log(ERROR, s, "Bad descriptor detected: %d\n", i);
                        }
                    }
                }
                ret = SELECTOR_IO;
...
void
selector_update_all_timeouts(fd_selector s) {
    for(size_t i = 0; i < s->fd_size; i++) {
        struct item *item = &(s->fds[i]);
        if(ITEM_USED(item)  &&  s->fds[i].data != NULL) {
            struct socks5 *s5 = (struct socks5*) s->fds[i].data;

            time_t now;
            time(&now);

            if(now - s5->last_update > STATE_TIMEOUT) {
                // Esta conexion estuvo parada mucho tiempo. Vamos a limpiarla
                struct selector_key key = {
                        .s    = s,
                        .fd   = item->fd,
                        .data = item->data,
                ...
                if(0 == item->handler->handle_timeout){
					assert(("OP_TIMEOUT arrived but no handler. bug!" == 0));
                }
                else {
