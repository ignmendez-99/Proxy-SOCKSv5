...
/**
 * Permite cambiar los intereses para un file descriptor. Si ya tenía algun interes, se añade el nuevo interes
 * sin eliminar el que ya tenia
 */
selector_status
selector_set_interest_additive(fd_selector s, int fd, fd_interest i);

/**
 * Elimina el interes @param i de la lista de intereses del @param fd
 */
selector_status
...
/** Si una conexión SOCKSv5 estuvo inactiva durante N segundos (ninguna parte envió data, el servidor tarda mucho en responder, etc.)
 *    esta función incrementará el tiemout de esta conexión en N.
 *  Si llega al máximo tiempo permitido de espera, entonces cierra la conexión
 */
void
