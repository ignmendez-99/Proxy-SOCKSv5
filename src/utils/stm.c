...
handle_first(struct state_machine *stm, struct selector_key *key) {
    if(stm->current == NULL) {
        ...
        if(NULL != stm->current->on_arrival) {
            return stm->current->on_arrival(stm->current->state, key);
        }
    }
    return stm->current->state;
}
...
void jump(struct state_machine *stm, unsigned next, struct selector_key *key) {
    ...
    if(stm->current != stm->states + next) {
        // If we are here, it means that the global state has changed.
        // We must finish current state and initialize the next one
...
stm_handler_timeout(struct state_machine *stm, struct selector_key *key){
	if(stm->current->on_timeout == 0){
		return STM_UNDEFINED_STATE;
	}

	const unsigned int ret = stm->current->on_timeout(key);
	jump(stm, ret, key);
